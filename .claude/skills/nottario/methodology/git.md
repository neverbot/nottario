...
One human, one Claude Code session at a time.
...
The human runs 2+ Claude Code sessions against the same repo at the
same time. Each session is working on a **different Nottario task**.
