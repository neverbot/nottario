...
<ul class="why-list">
  ...
  <li><strong>Docs the AI actually reads.</strong> The markdown store is served over MCP with versioning, so an agent can quote your <code>claude.md</code> back at you and edit it under optimistic concurrency instead of guessing.</li>
  ...
  <li><strong>Live web UI with per-user notifications.</strong> SSE + Postgres <code>LISTEN/NOTIFY</code> push changes as they land. A topbar bell surfaces assignments, comments and closures per user, opt-out per kind.</li>
